...
auto cholesky_GPU_1(sycl::queue &q, real_type *gpuA, size_t N, IDX index) {
    sycl::event e;
        ...
        e = q.submit([&](sycl::handler &cgh) {
            ...
            cgh.parallel_for(
                sycl::nd_range{ sycl::range(submatrixdim * WIDTH, submatrixdim), sycl::range(WIDTH, 1) }, e, [=](sycl::nd_item<2> idx) {
                    ...
                    const auto localj = idx.get_local_id(0);
                    ...
                    for (size_t u = 0; u < WIDTH; ++u) {
                        gpuA[index(offseti + u, offsetj + localj)] -= localCoefi[u] * localCoefj[localj];
                        sycl::group_barrier(idx.get_group());
                    }
                });
        });
...
int main(int argc, char **argv) {
    ...
    benchmarkCholesky<real_type>(A, [index](sycl::queue &q, real_type *gpuA) {
        cholesky_GPU_1<WIDTH>(q, gpuA, N, index);
    });
