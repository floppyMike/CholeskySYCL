...
constexpr static size_t THREADCOLWIDTH = 8;
constexpr static size_t THREADSUBWIDTH = 1;
constexpr static size_t THREADSUBHEIGHT = 1;
#else
using real_type = double;
constexpr static size_t THREADCOLWIDTH = 4;
constexpr static size_t THREADSUBWIDTH = 1;
constexpr static size_t THREADSUBHEIGHT = 1;
#endif
...
int main(int argc, char **argv) {
    sycl::queue q(sycl::cpu_selector_v, { sycl::property::queue::in_order(), sycl::property::queue::enable_profiling() });
    ...
    const auto kernel = [&q](size_t N, real_type *gpuA, real_type jitter) {
        ...
        for (size_t offset = 0; offset < Ndiv - 1; ++offset) {
            ...
            e = q.submit([&](sycl::handler &cgh) {
                ...
                auto cacheblock = sycl::local_accessor<real_type, 3>(sycl::range(THREADSUBHEIGHT, WIDTH, WIDTH), cgh);
                    ...
                    [=](sycl::nd_item<2> idx) {
                        ...
                        for (size_t k = 0; k < THREADSUBHEIGHT; ++k) {
                            if (auto offset = offseti + k; offset < blockwidth) {
                                cacheblock[k][locali][localj] = gpuA[idxgroup(locali, localj + offset * WIDTH)];
                            }
                        }
                        ...
                        for (size_t k = 0; k < WIDTH; ++k) {
                            real_type left[THREADSUBHEIGHT];
                            for (size_t u = 0; u < THREADSUBHEIGHT; ++u) {
                                left[u] = cacheblock[u][k][locali];
                            }
                            ...
                            for (size_t i = 0; i < THREADSUBHEIGHT; ++i) {
                                for (size_t j = 0; j < THREADSUBWIDTH; ++j) {
                                    sum[i * THREADSUBHEIGHT + j] += left[i] * top[j];
                                }
                            }
                        }
                                ...
                                for (size_t j = 0; j < THREADSUBWIDTH; ++j) {
                                    if (auto offsetjj = offsetj + j; offsetjj < blockwidth) {
                                        if (offsetii <= offsetjj) {
                                            gpuA[idxgroup(locali + offsetii * WIDTH, localj + offsetjj * WIDTH)] -= sum[i * THREADSUBHEIGHT + j];
                                        }
                                    }
                                }
                            }
    ...
    benchmarkCholesky<real_type>("06cpu", true, false, q, idx, gen, kernel);
    // for (size_t N = 8192; N <= 8192; ++N) {
