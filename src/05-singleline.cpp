...
void cholesky_decomposition(real_type *gpuA, real_type jitter, sycl::local_accessor<real_type, 2> cache, sycl::group<2> g, IDX idx) {
    ...
    for (size_t k = 0; k < WIDTH; ++k) {
        cache[k][localj] = gpuA[idx(k, localj)];
    }

    sycl::group_barrier(g);

    for (size_t k = 0; k < WIDTH - 1; ++k) {
        const auto top = cache[k][localj];
        const auto diag = cache[k][k] + jitter;
        for (size_t u = k + 1; u < WIDTH; ++u) {
            cache[u][localj] -= cache[k][u] * top / diag;
        }

        sycl::group_barrier(g);
    }

    const auto diag = cache[localj][localj] = sycl::sqrt(cache[localj][localj] + jitter);

    sycl::group_barrier(g);

    for (size_t k = 0; k < WIDTH - 1; ++k) {
        cache[k][localj] = (k == localj) * diag + (k != localj) * cache[k][localj] / cache[k][k];
    }
}
    ...
    const auto kernel = [&q](size_t N, real_type *gpuA, real_type jitter) {
        ...
        for (size_t offset = 0; offset < Ndiv - 1; ++offset) {
            e = q.submit([&](sycl::handler &cgh) {
                ...
                cgh.parallel_for(sycl::nd_range{ sycl::range(WIDTH, (Ndiv - 1 - offset) * WIDTH), sycl::range(1, WIDTH) }, [=](sycl::nd_item<2> idx) {
                    ...
                    for (size_t k = 0; k < WIDTH; ++k) {
                        cacheline[k][localj] = gpuA[idxgroup(k, localj + offsetj * WIDTH)];
                    }

                    sycl::group_barrier(idx.get_group());

                    for (size_t k = 0; k < WIDTH; ++k) {
                        const auto x = cacheline[k][localj] /= cacheblock[k][k];
                        for (size_t u = k + 1; u < WIDTH; ++u) {
                            cacheline[u][localj] -= cacheblock[k][u] * x;
                        }
                        sycl::group_barrier(idx.get_group());
                    }

                    for (size_t k = 0; k < WIDTH; ++k) {
                        gpuA[idxgroup(k, localj + offsetj * WIDTH)] = cacheline[k][localj];
                    }
                });
            });
        ...
        e = q.submit([&](sycl::handler &cgh) {
            ...
            cgh.parallel_for(sycl::nd_range{ sycl::range(WIDTH, Naligned), sycl::range(1, WIDTH) }, [=](sycl::nd_item<2> idx) {
                const auto g = idx.get_group();
                ...
                for (size_t k = 0; k < WIDTH; ++k) {
                    gpuA[idxgroup(k, localj)] = cache[k][localj];
                }
            });
        });
        times.push_back(getduration(e));
