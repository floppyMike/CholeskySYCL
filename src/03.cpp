...
int main(int argc, char **argv) {
    ...
    const auto kernel = [&q](size_t N, real_type *gpuA, real_type jitter) {
        ...
        for (size_t _k = 1; _k < N; ++_k) {
            ...
            e = q.parallel_for(sycl::range<2>(N - _k, N - _k), [=](sycl::id<2> idx) {
                ...
                if (i >= j) {
                    gpuA[row_idx(i, j)] -= gpuA[row_idx(i, k)] * gpuA[row_idx(j, k)] / (gpuA[row_idx(k, k)] + jitter);
                }
            });
        ...
        e = q.parallel_for(sycl::range<2>(N, N), [=](sycl::id<2> idx) {
            const auto i = idx[0];
            const auto j = idx[1];
            if (i > j) {
                gpuA[row_idx(i, j)] /= gpuA[row_idx(j, j)];
            }
        });
        times.push_back(getduration(e));
    ...
    benchmarkCholesky<real_type>("03", true, false, q, idx, row_idx_alloc, kernel);
